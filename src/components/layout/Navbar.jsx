        ...
        <div className='max-w-[1320px] mx-auto px-5'>
            <div className='flex items-center justify-between'>
                ...
                <nav className='hidden md:flex items-center gap-7'>
                    {NAV_LINKS.map((link) => (
                        <button
                            ...
                            className={`text-base font-medium transition-all cursor-pointer duration-300 ${
                ...
                <button
                    ...
                    className='md:hidden p-4 text-white hover:text-white/80 transition-colors cursor-pointer'
                    aria-label='menu'
            ...
            <div className="bg-black/95 backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-3">
                {NAV_LINKS.map((link) => (
                    <button
                        key={link.id}
                        onClick={() => handleNavClick(link.id)}
                        className={`block w-full text-center px-4 py-3 rounded-lg font-medium transition-all duration-300 cursor-pointer ${
                        activeSection === link.id ? 'text-white bg-white/10' : 'text-white/70 hover:text-white hover:bg-white/10'
                        }`}
                    >
                        {link.label}
                    </button>
                ...
                <button
                    onClick={() => handleNavClick('contact')}
                    className="w-full px-7 py-3.5 bg-white text-[#212121] font-medium text-base rounded-[17px] border border-white hover:bg-white/90 transition-all duration-300 mt-2 cursor-pointer"
                >
                    Hire Me
                </button>
