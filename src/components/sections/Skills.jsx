...
const Skills = () => {
    ...
    const skillCategories = {
        ...
        skills.find(s => s.name === 'JavaScript (ES6+)'),
        skills.find(s => s.name === 'Bootstrap'),
        skills.find(s => s.name === 'AJAX'),
        skills.find(s => s.name === 'Tailwind CSS'),
        skills.find(s => s.name === 'React.js'),
        skills.find(s => s.name === 'React Router'),
        skills.find(s => s.name === 'React Hooks'),
        ...
        skills.find(s => s.name === 'SEO'),
        skills.find(s => s.name === 'Google Analytics'),
        skills.find(s => s.name === 'Social Media Marketing'),
        skills.find(s => s.name === 'Social Media Campaigning'),
        skills.find(s => s.name === 'Content & Email Marketing'),
        skills.find(s => s.name === 'Graphic Designing & Video Editing'),
  ...
  return <section id="skills" className="relative py-20 bg-black overflow-hidden">
            ...
            <div className="absolute inset-0 overflow-hidden">
                <div className="absolute top-1/4 left-0 w-96 bg-primary/10 rounded-full blur-3xl opacity-50"></div>
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <FadeIn delay={100}>
                    <div className="text-center mb-16">
                        ...
                        <p className="text-lg text-white/60 max-w-2xl mx-auto">
                            A detailed summary of my technical capabilities, demonstrating proficiency in industry-relevant technologies and best practices.
                        </p>
                    </div>
                </FadeIn>
                {/* Skills Categories  */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {Object.entries(skillCategories).map(([category, categorySkills], categoryIndex) =>
                        <FadeIn key={category} delay={categoryIndex *10}>
